...
describe('RegisterStudentUseCase - 1.1', () => {
  test('Should call LoadStudentByEmail with provided email', async () => {
    ...
    expect(loadByEmail).toHaveBeenCalledTimes(1)
    expect(loadByEmail).toHaveBeenCalledWith(expect.objectContaining({ value: 'test@example.com' }))
  })
})
